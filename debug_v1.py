...
from weights import StabilityScore_v1
...
def main() -> None:
    ...
    if ETA_OVERRIDE is not None:
        ss_kwargs["eta"] = float(ETA_OVERRIDE)

    result = StabilityScore_v1.StabilityScore(NPZ_PATH, **ss_kwargs).compute()
    ...
    tag = f"{meta['dataset']}_{meta['model']}_E{E}_w{window_used}_p{P0:.2f}_tau{TAU:.2f}_v1"
    _ensure_dir(OUT_DIR)
    ...
    # Additional stats for v1
    print("\n=== Stats: v1 gain diagnostics ===")
    _print_stats("S_global [all]", _describe(S_global))
    _print_stats("gain [all]", _describe(gain))
    ...
    _save_hist(
        ...
        out_path=OUT_DIR / f"{tag}__v1__score_all.png",
        bins=40,
    )
    _save_hist(
        L,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__v1__L_all.png",
        bins=40,
    )
    _save_hist(
        S,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__v1__S_all.png",
        bins=40,
    )
    _save_hist(
        S_global,
        title=f"S_global distribution ({tag})",
        xlabel="S_global",
        out_path=OUT_DIR / f"{tag}__v1__S_global_all.png",
        bins=40,
    )
    _save_hist(
        gain,
        title=f"gain distribution ({tag})",
        xlabel="gain",
        out_path=OUT_DIR / f"{tag}__v1__gain_all.png",
        bins=40,
    )
    ...
    _save_hist(
        ...
        xlabel="StabilityScore",
        out_path=OUT_DIR / f"{tag}__v1__score_learnable.png",
        bins=40,
    )
    _save_hist(
        score_non,
        ...
        xlabel="StabilityScore",
        out_path=OUT_DIR / f"{tag}__v1__score_nonlearnable.png",
        bins=40,
    )
    _save_hist(
        L_learn,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__v1__L_learnable.png",
        bins=40,
    )
    _save_hist(
        L_non,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__v1__L_nonlearnable.png",
        bins=40,
    )
    _save_hist(
        S_learn,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__v1__S_learnable.png",
        bins=40,
    )
    _save_hist(
        S_non,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__v1__S_nonlearnable.png",
        bins=40,
    )
    ...
    _save_hist(
        ...
        xlabel="M_i",
        out_path=OUT_DIR / f"{tag}__v1__M_all.png",
        bins=40,
    ...
    _save_hist(
        ...
        xlabel="I_i",
        out_path=OUT_DIR / f"{tag}__v1__I_all.png",
        bins=40,
        vlines=[(0.5, "0.5")],
    )
    _save_hist(
        ...
        xlabel="M_i",
        out_path=OUT_DIR / f"{tag}__v1__M_nonlearnable.png",
        bins=40,
        vlines=[(P0, f"p0={P0:.2f}")],
    )
    ...
    _save_scatter(
        ...
        xlabel="L_i",
        ylabel="S_i",
        out_path=OUT_DIR / f"{tag}__v1__scatter_LS_learnable.png",
    )
    # All-samples scatter
    _save_scatter(
        L,
        ...
        xlabel="L_i",
        ylabel="S_i",
        out_path=OUT_DIR / f"{tag}__v1__scatter_LS_all.png",
    )
