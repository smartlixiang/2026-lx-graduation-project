...
from weights.StabilityScore_v4 import StabilityScoreV4
...
# (We still plot M_i and show threshold lines.)
...
def main() -> None:
    ...
    result = StabilityScoreV4(NPZ_PATH, **ss_kwargs).compute()
    ...
    tag = f"{meta['dataset']}_{meta['model']}_E{E}_p{P0:.2f}_tau{TAU:.2f}"
    _ensure_dir(OUT_DIR)
    ...
    # ---- Compute M_i and I_i for diagnostics
    window_used = 3
    correct = correct.astype(bool)
    if correct.shape[0] >= 3:
        window_used = 3
    M = _max_window_acc(correct, window_used)
    I = _sigmoid((M - P0) / TAU)

    # =========================
    # Quantiles / ratios required by v4
    # =========================
    print("\n=== Quantiles (v4) ===")
    _print_quantiles("S (post_stability)", S)
    _print_quantiles("L (learn_time_normalized)", L)
    _print_quantiles("g (soft gate)", g)
    _print_quantiles("scores", scores)
    print(f"g>0.5 ratio: {float((g > 0.5).mean()):.4f}")
    ...
    _save_hist(
        ...
        out_path=OUT_DIR / f"{tag}__score_all_v4.png",
        bins=40,
    )
    _save_hist(
        L,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__L_all_v4.png",
        bins=40,
    )
    _save_hist(
        S,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__S_all_v4.png",
        bins=40,
    )
    ...
    _save_hist(
        ...
        xlabel="StabilityScore",
        out_path=OUT_DIR / f"{tag}__score_learnable_v4.png",
        bins=40,
    )
    _save_hist(
        score_non,
        ...
        xlabel="StabilityScore",
        out_path=OUT_DIR / f"{tag}__score_nonlearnable_v4.png",
        bins=40,
    )
    _save_hist(
        L_learn,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__L_learnable_v4.png",
        bins=40,
    )
    _save_hist(
        L_non,
        ...
        xlabel="L_i",
        out_path=OUT_DIR / f"{tag}__L_nonlearnable_v4.png",
        bins=40,
    )
    _save_hist(
        S_learn,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__S_learnable_v4.png",
        bins=40,
    )
    _save_hist(
        S_non,
        ...
        xlabel="S_i",
        out_path=OUT_DIR / f"{tag}__S_nonlearnable_v4.png",
        bins=40,
    )
    ...
    _save_hist(
        ...
        xlabel="M_i",
        out_path=OUT_DIR / f"{tag}__M_all_v4.png",
        bins=40,
    ...
    _save_hist(
        ...
        xlabel="I_i",
        out_path=OUT_DIR / f"{tag}__I_all_v4.png",
        bins=40,
        vlines=[(0.5, "0.5")],
    )
    _save_hist(
        ...
        xlabel="M_i",
        out_path=OUT_DIR / f"{tag}__M_nonlearnable_v4.png",
        bins=40,
        vlines=[(P0, f"p0={P0:.2f}")],
    )

    # v4-specific histograms
    _save_hist(
        g,
        title=f"g(S) distribution ({tag})",
        xlabel="g",
        out_path=OUT_DIR / f"{tag}__g_all_v4.png",
        bins=40,
        vlines=[(0.5, "g=0.5 (S=S0)")],
    )
    _save_hist(
        score_learn,
        title=f"Score_learn distribution ({tag})",
        xlabel="Score_learn",
        out_path=OUT_DIR / f"{tag}__score_learn_all_v4.png",
        bins=40,
    )
    _save_hist(
        score_unlearn,
        ...
        out_path=OUT_DIR / f"{tag}__score_unlearn_all_v4.png",
        bins=40,
    )
    ...
    _save_scatter(
        ...
        xlabel="L_i",
        ylabel="S_i",
        out_path=OUT_DIR / f"{tag}__scatter_LS_learnable_v4.png",
    )
    # All-samples scatter
    _save_scatter(
        L,
        ...
        xlabel="L_i",
        ylabel="S_i",
        out_path=OUT_DIR / f"{tag}__scatter_LS_all_v4.png",
    )
    # v4 scatter: S vs g
    _save_scatter(
        S,
        g,
        scores,
        title=f"S vs g (all), color=score ({tag})",
        xlabel="S_i",
        ylabel="g",
        out_path=OUT_DIR / f"{tag}__scatter_Sg_all_v4.png",
    )
