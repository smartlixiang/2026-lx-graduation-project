...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize dynamic v2 component distributions.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional direct path to dynamic_components_v2.npz.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output image path. Default: result/{dataset}_dynamic_components_v2_hist.png",
    )
...
def main() -> None:
    ...
    if not cache_path.exists():
        raise FileNotFoundError(
            f"Dynamic v2 cache not found: {cache_path}. "
            "Please run learn_scoring_weights.py first to generate dynamic_components_v2.npz."
        )

    data = np.load(cache_path, allow_pickle=True)
    required = ["A_norm2", "C_norm2", "G_norm2", "D_norm2"]
    missing = [k for k in required if k not in data]
    if missing:
        raise KeyError(f"Missing required keys in cache {cache_path}: {missing}")

    components = {
        "A: Early Learnability": data["A_norm2"].astype(np.float32),
        "C: Dynamic Class Complementarity": data["C_norm2"].astype(np.float32),
        "G: OOF Support": data["G_norm2"].astype(np.float32),
        "D: OOF Pattern Gap": data["D_norm2"].astype(np.float32),
    }
    ...
    fig.suptitle(f"Dynamic v2 components histogram | dataset={args.dataset}, seed={args.seed}")

    output_path = (
        Path(args.output)
        if args.output is not None
        else Path("result") / f"{args.dataset}_dynamic_components_v2_hist.png"
    )
