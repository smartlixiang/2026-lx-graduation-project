...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Naive scoring selection effectiveness (CIFAR-10)")
    ...
    parser.add_argument("--adapter-path", type=str, default=None, help="可选 adapter 权重路径")
...
def ensure_scoring_weights(path: Path, dataset_name: str) -> dict[str, float]:
    ...
    return {k: float(v) for k, v in naive.items()}
...
def main() -> None:
    ...
    result_dir = PROJECT_ROOT / "test_naive_scoring_result"
    result_dir.mkdir(parents=True, exist_ok=True)
    result_path = result_dir / f"naive_scoring_cr{args.cr}.json"
    result_payload = {
        "cr": args.cr,
