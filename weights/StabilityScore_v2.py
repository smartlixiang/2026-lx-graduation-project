"""StabilityScore v2 with logits-based post stability."""
...
@dataclass
class StabilityResultV2:
    """Container for StabilityScore v2 outputs."""
...
class StabilityScore:
    """Compute StabilityScore v2 from proxy training logs (.npz)."""
    ...
    def compute(self) -> StabilityResultV2:
        if self.window <= 0:
        ...
        if "logits" not in data:
            raise ValueError("proxy log must include 'logits' for StabilityScore v2.")
        if "labels" not in data:
            raise ValueError("proxy log must include 'labels' for StabilityScore v2.")
        ...
        p_true = self._prob_true(logits, labels)
...
__all__ = ["StabilityResultV2", "StabilityScore"]
