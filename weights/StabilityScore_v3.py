"""StabilityScore v3 with logits-based stability and EMA learn time."""
...
@dataclass
class StabilityResultV3:
    """Container for StabilityScore v3 outputs."""
...
class StabilityScore:
    """Compute StabilityScore v3 from proxy training logs (.npz)."""
    ...
    @staticmethod
    def _prob_true(logits: np.ndarray, labels: np.ndarray) -> np.ndarray:
        logits = logits.astype(np.float32)
        max_logits = np.max(logits, axis=2, keepdims=True)
        exp_logits = np.exp(logits - max_logits)
        sum_exp = np.sum(exp_logits, axis=2)
        num_epochs, num_samples, _ = logits.shape
        label_idx = np.broadcast_to(labels.reshape(1, num_samples, 1), (num_epochs, num_samples, 1))
        true_exp = np.take_along_axis(exp_logits, label_idx, axis=2).squeeze(2)
        return true_exp / (sum_exp + np.finfo(np.float32).eps)

    @staticmethod
    def _ema(values: np.ndarray, beta: float) -> np.ndarray:
        ema = np.zeros_like(values, dtype=np.float32)
        ema[0] = values[0]
        for t in range(1, values.shape[0]):
            ema[t] = beta * ema[t - 1] + (1.0 - beta) * values[t]
        return ema

    def compute(self) -> StabilityResultV3:
        if self.window <= 0:
        ...
        if "logits" not in data:
            raise ValueError("proxy log must include 'logits' for StabilityScore v3.")
        if "labels" not in data:
            raise ValueError("proxy log must include 'labels' for StabilityScore v3.")
        labels = data["labels"].astype(np.int64)
        logits = data["logits"]
        self._validate_logits(logits, labels)

        correct, _ = self._build_correct(data)
        ...
        p_true = self._prob_true(logits, labels).astype(np.float32)
        ema = self._ema(p_true, self.beta)
        a_max = ema.max(axis=0)
        has_reach = ema >= self.theta
        t_reach0 = np.argmax(has_reach, axis=0)
        t_star0 = np.argmax(ema, axis=0)
        reached_mask = has_reach.any(axis=0)
        t_anchor0 = np.where(reached_mask, t_reach0, t_star0).astype(np.int32)
        ...
        cumsum_tail = np.cumsum(p_true[::-1], axis=0)[::-1]
        cumsum_tail_sq = np.cumsum((p_true ** 2)[::-1], axis=0)[::-1]
        lengths = np.arange(num_epochs, 0, -1, dtype=np.float32)
        sample_indices = np.arange(num_samples)

        mu_post = cumsum_tail[t_anchor0, sample_indices] / lengths[t_anchor0]
        mean_sq = cumsum_tail_sq[t_anchor0, sample_indices] / lengths[t_anchor0]
        var_post = np.maximum(mean_sq - mu_post**2, 0.0)
        sigma_post = np.sqrt(var_post)
        s_raw = mu_post - self.lam * sigma_post

        lo = float(np.percentile(s_raw, 1))
        hi = float(np.percentile(s_raw, 99))
        denom_norm = hi - lo + np.finfo(np.float32).eps
        post_stability = np.clip((s_raw - lo) / denom_norm, 0.0, 1.0).astype(np.float32)
        ...
        if not np.array_equal(indices, np.arange(len(indices))):
            ...
            s_raw = s_raw[order]
            a_max = a_max[order]
            t_anchor0 = t_anchor0[order]
            indices = indices[order]
            labels = labels[order]

        return StabilityResultV3(
            scores=scores,
...
__all__ = ["StabilityResultV3", "StabilityScore"]
