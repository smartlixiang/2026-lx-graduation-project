"""StabilityScore v5 implementation based on proxy training dynamics."""
...
@dataclass
class StabilityResultV5:
    """Container for StabilityScore v5 outputs."""
...
class StabilityScoreV5:
    ...
    def _load_logits(self, data: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        if "logits" not in data:
            raise ValueError("proxy log must include 'logits' for v5 computation.")
    ...
    def compute(self) -> StabilityResultV5:
...
__all__ = ["StabilityResultV5", "StabilityScoreV5"]
